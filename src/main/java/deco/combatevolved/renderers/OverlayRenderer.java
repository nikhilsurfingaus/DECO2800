...
public class OverlayRenderer implements Renderer {
	...
	private void renderDebugText(SpriteBatch batch, Camera camera) {
		...
		debugLine(batch, camera, line++, "== Weather Condition ==");
		debugLine(batch, camera, line++,
				String.format("Current Weather State: %s", GameManager.get().getWorld().getWeather().getWeatherState()));
		debugLine(batch, camera, line++, String.format("Current Forecast: %s", GameManager.get().getWorld().getWeather().getForecastString()));
		debugLine(batch, camera, line++, String.format("Climate Factor: %s", GameManager.get().getWorld().getWeather().getClimateFactor()));

    	debugLine(batch, camera, line++, "== Time of Day ==");
    	debugLine(batch, camera, line++,
				String.format("Cycle code: %s", GameManager.get().getManager(DayNightCycle.class).getCycleCode()));
    }
